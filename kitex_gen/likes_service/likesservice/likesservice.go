...
func isLikedHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	...
	case *streaming.Args:
		st := s.Stream
		req := new(likes_service.IsLikedRequest)
		if err := st.RecvMsg(req); err != nil {
			return err
		}
		resp, err := handler.(likes_service.LikesService).IsLiked(ctx, req)
		if err != nil {
			return err
		}
		return st.SendMsg(resp)
	case *IsLikedArgs:
		success, err := handler.(likes_service.LikesService).IsLiked(ctx, s.Req)
		if err != nil {
			return err
		}
		realResult := result.(*IsLikedResult)
		realResult.Success = success
		return nil
	default:
		return errInvalidMessageType
...
func newIsLikedArgs() interface{} {
	return &IsLikedArgs{}
}

func newIsLikedResult() interface{} {
	return &IsLikedResult{}
}

type IsLikedArgs struct {
	Req *likes_service.IsLikedRequest
}

func (p *IsLikedArgs) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetReq() {
		p.Req = new(likes_service.IsLikedRequest)
	}
	return p.Req.FastRead(buf, _type, number)
}

func (p *IsLikedArgs) FastWrite(buf []byte) (n int) {
	if !p.IsSetReq() {
		return 0
	}
	return p.Req.FastWrite(buf)
...
func (p *IsLikedArgs) Marshal(out []byte) ([]byte, error) {
	if !p.IsSetReq() {
		return out, nil
	}
	return proto.Marshal(p.Req)
...
func (p *IsLikedArgs) Unmarshal(in []byte) error {
	msg := new(likes_service.IsLikedRequest)
	if err := proto.Unmarshal(in, msg); err != nil {
		return err
	}
	p.Req = msg
...
var IsLikedArgs_Req_DEFAULT *likes_service.IsLikedRequest

func (p *IsLikedArgs) GetReq() *likes_service.IsLikedRequest {
	if !p.IsSetReq() {
		return IsLikedArgs_Req_DEFAULT
	}
	return p.Req
}

func (p *IsLikedArgs) IsSetReq() bool {
	return p.Req != nil
}

func (p *IsLikedArgs) GetFirstArgument() interface{} {
	return p.Req
}

type IsLikedResult struct {
	Success *likes_service.IsLikedResponse
}

var IsLikedResult_Success_DEFAULT *likes_service.IsLikedResponse

func (p *IsLikedResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(likes_service.IsLikedResponse)
	}
	return p.Success.FastRead(buf, _type, number)
}

func (p *IsLikedResult) FastWrite(buf []byte) (n int) {
	if !p.IsSetSuccess() {
		return 0
	}
	return p.Success.FastWrite(buf)
...
func (p *IsLikedResult) Size() (n int) {
	if !p.IsSetSuccess() {
		return 0
	}
	return p.Success.Size()
...
func (p *IsLikedResult) Marshal(out []byte) ([]byte, error) {
	if !p.IsSetSuccess() {
		return out, nil
	}
	return proto.Marshal(p.Success)
...
func (p *IsLikedResult) Unmarshal(in []byte) error {
	msg := new(likes_service.IsLikedResponse)
	if err := proto.Unmarshal(in, msg); err != nil {
		return err
	}
	p.Success = msg
...
func (p *IsLikedResult) GetSuccess() *likes_service.IsLikedResponse {
	if !p.IsSetSuccess() {
		return IsLikedResult_Success_DEFAULT
	}
	return p.Success
}

func (p *IsLikedResult) SetSuccess(x interface{}) {
	p.Success = x.(*likes_service.IsLikedResponse)
}

func (p *IsLikedResult) IsSetSuccess() bool {
	return p.Success != nil
}

func (p *IsLikedResult) GetResult() interface{} {
	return p.Success
}
...
func (p *kClient) IsLiked(ctx context.Context, Req *likes_service.IsLikedRequest) (r *likes_service.IsLikedResponse, err error) {
	var _args IsLikedArgs
	_args.Req = Req
	var _result IsLikedResult
